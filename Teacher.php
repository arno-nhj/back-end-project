...
<h3>See all teachers</h3>
		<table>
			<tr>
				...
				<th width="250px">Teacher background<br><hr></th>
			</tr>
			<?php
			 $sql = mysqli_query($link, "SELECT TEACHER_ID,TEAHCER_NAME,TEACHER_ADDRESS,TEACHER_SALARY,TEACHER_BG FROM teacher");
			while ($row = $sql->fetch_assoc()){
			echo "
			<tr>
				<th>{$row['TEACHER_ID']}</th>
				<th>{$row['TEAHCER_NAME']}</th>
				<th>{$row['TEACHER_ADDRESS']}</th>
				<th>{$row['TEACHER_SALARY']}</th>
				<th>{$row['TEACHER_BG']}</th>
			</tr>";
			}
			?>
        </table>
